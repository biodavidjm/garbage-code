...
my $script_name = "djmusic_parser01.pl";
            ...
            if ($name) {
                if ($band) {
                    if ($album) {
                        ...
                        my $artist_object = Band->new(
                            band_name => $band,
                        );

                        print "Artist: ", $artist_object->band_name;
                        say " Country: ", $artist_object->set_country("France");
                        # say " (but if I assign country: ", $artist_object->country("England"),")";
                        exit;
...
# OUTPUT FILE
my $outfile = "pop_all_djmusic_tables01.sql";
...
djmusic_parser01.pl - Extract music info from iTunes XML file

=head1 SYNOPSIS

perl djmusic_parser01.pl -f=djm_itunes2014grouping_proc.xml
